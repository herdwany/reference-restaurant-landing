...
const parseUpdateInput = (input, restaurantId) => {
    const allowedFields = ["plan", "status", "billingStatus", "supportLevel", "subscriptionEndsAt", "trialEndsAt"];
    const updates = {};
    const updateFields = [];
    ...
    for (const field of allowedFields) {
        if (field in input) {
            ...
            if (value === null || value === undefined) {
                ...
                const cleanedValue = cleanText(value, 255);
                if (cleanedValue) {
...
const validateAllowedEnumValues = (updates) => {
    const allowedEnums = {
        ...
        billingStatus: ["unpaid", "paid", "overdue", "trial"],
        supportLevel: ["basic", "standard", "premium", "none"],
    };
...
const logAudit = async (tablesDb, userId, restaurantId, action, changes, status = "success") => {
    try {
        ...
        // Log warning but do not break the main operation
...
export default async ({ req, res, log, error }) => {
    try {
        ...
        // Get current restaurant to verify it exists and caller has access
        ...
        // Log audit
        await logAudit(tablesDb, userId, restaurantId, "updateClientControls", {
            updatedFields,
            changes: updates,
        });

        log(`Agency admin ${userId} updated client controls for restaurant ${restaurantId}`);
        ...
        const message = caughtError instanceof Error ? caughtError.message : "Failed to update client controls.";
        ...
        return json(
            ...
            {
                ok: false,
                message: status >= 500 ? "Failed to update client controls. Please try again." : message,
            },
            status,
        );
    }
