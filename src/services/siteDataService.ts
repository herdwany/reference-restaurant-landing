...
const mergeSettings = (settings: SiteSettings | null, base: RestaurantConfig): RestaurantConfig["settings"] => {
  if (!settings) {
    return {
      ...
      deliveryEnabled: base.settings.deliveryEnabled ?? true,
      pickupEnabled: base.settings.pickupEnabled ?? false,
      deliveryBaseFee: base.settings.deliveryBaseFee ?? base.restaurant.deliveryFee,
      freeDeliveryThreshold: base.settings.freeDeliveryThreshold,
      minimumOrderAmount: base.settings.minimumOrderAmount,
      estimatedDeliveryMinutes: base.settings.estimatedDeliveryMinutes,
      deliveryAreas: base.settings.deliveryAreas,
      deliveryInstructions: base.settings.deliveryInstructions,
  ...
  return {
    ...
    deliveryEnabled: settings.deliveryEnabled ?? base.settings.deliveryEnabled ?? true,
    pickupEnabled: settings.pickupEnabled ?? base.settings.pickupEnabled ?? false,
    deliveryBaseFee: settings.deliveryBaseFee ?? base.settings.deliveryBaseFee ?? base.restaurant.deliveryFee,
    freeDeliveryThreshold: settings.freeDeliveryThreshold ?? base.settings.freeDeliveryThreshold,
    minimumOrderAmount: settings.minimumOrderAmount ?? base.settings.minimumOrderAmount,
    estimatedDeliveryMinutes: settings.estimatedDeliveryMinutes?.trim() || base.settings.estimatedDeliveryMinutes,
    deliveryAreas: settings.deliveryAreas?.trim() || base.settings.deliveryAreas,
    deliveryInstructions: settings.deliveryInstructions?.trim() || base.settings.deliveryInstructions,
